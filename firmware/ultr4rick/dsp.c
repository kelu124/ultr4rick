...
void u4rk_dsp_envelope(const uint16_t *dma_samples, uint16_t *raw_out,
     ...
     * FFT does the forward transform with half the complex FFT work. */
    arm_rfft_fast_f32(&rfft, rfft_buffer, envelope_buffer, 0);
    metrics->forward_fft_us = elapsed_us(stage_started);

    stage_started = time_us_64();
    /* Build the spectrum of the real Hilbert transform. Multiplication by
     * -j maps each positive-frequency value (real + j*imag) to
     * (imag - j*real). DC and Nyquist have no Hilbert component. */
...
void u4rk_dsp_make_selftest(uint8_t test_case, uint16_t *dma_samples) {
    /* Select the case outside the loop so zero and DC do not execute 4096
     * unnecessary trigonometric calls before their first USB frame. */
    if (test_case == 0u) {
