...
static void send_status(void) {
    ...
    send_ok(
        "board=ultr4rick package=RP2350B firmware=%s "
        ...
        "dsp_us=%u worst_us=%u performance=%s alaw_max_rate=%u cmsis=%s",
        ...
        metrics.worst_total_us <= U4RK_DSP_TARGET_US ? "ok" : "over-budget",
        U4RK_ALAW_MAX_RATE_HZ,
...
static void process_command(char *line) {
    ...
    if (strcmp(first, "pulse") == 0 && second != NULL &&
        ...
        if (!u4rk_pulser_configure(
            ...
            send_error("RANGE", "minimum is 16/32/16 ns");
            return;
        }
...
int main(void) {
    bi_decl(bi_program_description(
        "Ultr4rick 4096-sample Hilbert envelope and A-law firmware"));
    ...
    bi_decl(bi_3pins_with_names(
        ...
        U4RK_DAC_SCK_PIN, "MCP4812 SCK jumper",
        U4RK_DAC_TX_PIN, "MCP4812 MOSI jumper"));
    bi_decl(bi_2pins_with_names(
        U4RK_PULSER_PIN_BASE, "Pulser P+",
        U4RK_PULSER_PIN_BASE + 1u, "Pulser PDAMP"));

    /*
     * Pulser safety comes first: both control pins are driven to 00 before
